...
<head>
	...
	<script>        
        ...
        $(document).on('blur', '#lista_cod_producto', function() {
            var cod_producto = $(this).data("cod_producto");
            
            actualizarProductos(cod_producto, cod_producto, "cod_product");
        });
        
        $(document).on('blur', '#lista_nombre_producto', function() {
        ...
        $(document).on('blur', '#lista_cantidad_producto', function() {
        ...
        $(document).on('blur', '#lista_valor_unitario', function() {
            var cod_producto = $(this).data("valor_unitario");
            var valor_unitario = $(this).value();
            
            actualizarProductos(cod_producto, valor_unitario, "unit_value");
        });
        
        $(document).on('change', '#lista_nombre_proveedor option:selected', function() {
            $('#lista_nombre_proveedor option:selected')
            var cod_producto = $(this).data("nombre_proveedor");
            var nombre_proveedor = $(this).text();
            
            actualizarProductos(cod_producto, nombre_proveedor, "name_provider");
        });
					...
					<table class="w3-table-all w3-hoverable focus">
						...
						<?php
							foreach($productos as $fila){
								...
								echo "<td id='lista_valor_unitario' data-valor_unitario='".$fila -> get ('cod_product')."' contenteditable>".$fila -> get ('unit_value')."</td>";               echo "<td>";
                                ?>
                                    <select id='lista_nombre_proveedor' data-nombre_proveedor='<?php echo $fila -> get ('cod_product'); ?>' class='w3-select' name='editar_lista_proveedores'>
                                        <option value="" disabled>Proveedores</option>
                                        <?php    
                                        foreach($proveedores as $row){
                                            if($fila -> get ('name_provider') == $row -> get('name_provider')){
                                                echo "<option id='lista_nombre_proveedor' value=".$row -> get ('cod_provider')." selected>".$row -> get ('name_provider')."</option>";
                                            }else{
